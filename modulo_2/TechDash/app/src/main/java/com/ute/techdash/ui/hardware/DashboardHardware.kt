...
fun DashboardHardware() {
    ...
    val estadoRedFlow = remember(repositorioRed) { repositorioRed.conectividadFlow() }
    val estadoRed by estadoRedFlow
        .collectAsStateWithLifecycle(initialValue = repositorioRed.estadoActual())

    val acelerometroFlow = remember(repositorioSensores) {
        repositorioSensores.acelerometroFlow()
            .catch { emit(DatoAcelerometro(0f, 0f, 0f)) }
    }
    val acelerometro by acelerometroFlow.collectAsStateWithLifecycle(initialValue = null)

    val luzFlow = remember(repositorioSensores) {
        repositorioSensores.luzAmbienteFlow()
            .catch { emit(DatoLuz(0f)) }
    }
    val luz by luzFlow.collectAsStateWithLifecycle(initialValue = null)
